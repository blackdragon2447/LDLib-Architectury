...
public final class CompassManager implements ResourceManagerReloadListener {
	...
	public void init() {
		...
		registerComponent("table", TableComponent::new);
	...
	public void onResourceManagerReload(@Nonnull ResourceManager resourceManager) {
		...
		for (var entry : resourceManager.listResources("compass/sections", rl -> rl.getPath().endsWith(".json"))
				.entrySet()) {
			...
			try (var reader = new InputStreamReader(resource.open(), StandardCharsets.UTF_8)) {
				...
				var section = new CompassSection(new ResourceLocation(key.getNamespace(), path),
		...
		for (var entry : resourceManager.listResources("compass/nodes", rl -> rl.getPath().endsWith(".json"))
				.entrySet()) {
			var key = entry.getKey();
			var resource = entry.getValue();
			try (var reader = new InputStreamReader(resource.open(), StandardCharsets.UTF_8)) {
				...
				path = path.substring(0, path.length() - 5);
				var node = new CompassNode(new ResourceLocation(key.getNamespace(), path),
						JsonParser.parseReader(reader).getAsJsonObject());
		...
		for (var entry : nodes.entrySet()) {
			...
			while (iterator.hasNext()) {
				...
				if (section != null) {
					...
					node.getItems()
							.forEach(item -> itemLookup.computeIfAbsent(item, k -> new HashSet<>()).add(node.nodeName));
				} else {
	...
	public void openCompass(ResourceLocation nodeLocation) {
		var node = nodes.getOrDefault(nodeLocation.getNamespace(), Collections.emptyMap()).getOrDefault(nodeLocation,
				null);
		if (node != null) {
			openCompass(node);
		}
	...
	public void openCompass(CompassNode... compassNodes) {
		...
		if (compassNodes.length == 1) {
			...
			uiTemplate = new ModularUI(210, 100, holder, entityPlayer)
					.widget(new CompassSelectorWidget(Arrays.asList(compassNodes)));
		} else {
			return;
		}
	...
	public List<CompassNode> getNodesByItem(Item item) {
		return itemLookup.getOrDefault(item, Collections.emptySet()).stream()
				.map(nodeName -> nodes.getOrDefault(nodeName.getNamespace(), Collections.emptyMap()).get(nodeName))
				.toList();
	}
